...
function testConnection() {
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            let errorMessage = data.message || 'সংযোগ ব্যর্থ';
            
            // Show detailed diagnostics if available
            if (data.diagnostics) {
                let diagnosticsHtml = '<div class="mt-3 space-y-2 text-sm">';
                diagnosticsHtml += '<strong>বিস্তারিত তথ্য:</strong><br>';
                
                Object.keys(data.diagnostics).forEach(key => {
                    const diag = data.diagnostics[key];
                    const icon = diag.success ? '✅' : '❌';
                    diagnosticsHtml += `${icon} ${diag.message}<br>`;
                });
                
                if (data.connection_details && data.connection_details.suggestion) {
                    diagnosticsHtml += `<div class="mt-2 p-2 bg-yellow-100 rounded text-yellow-800">
                        <strong>পরামর্শ:</strong> ${data.connection_details.suggestion}
                    </div>`;
                }
                ...
                const statusDiv = document.getElementById('connection-status');
                statusDiv.innerHTML = `
                    <div class="rounded-lg border p-4 bg-red-50 border-red-200 text-red-800">
